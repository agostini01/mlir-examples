...
func @main() {
  ...
  %input = call @alloc_4d_filled_f32(%c1, %c5, %c5, %c1, %v1) : 
    (index, index, index, index, f32) -> (memref<?x?x?x?xf32>)

  %filter = call @alloc_4d_filled_f32(%c3, %c3, %c1, %c1, %v3) :
    (index, index, index, index, f32) -> (memref<?x?x?x?xf32>) 
...
//                  N   H   W   O   C   kH  kW      N , H  + kH +1, W  + kW +1, C
#map1 = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d0, d1 + d5 -1, d2 + d6 -1, d4)>
